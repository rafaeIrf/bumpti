-- Add age range filtering to active users functions
-- Filter users based on requesting user's age_range_min and age_range_max preferences

-- PART 1: Update get_active_users_with_avatars to filter by age range
...
-- PART 2: Update get_eligible_active_users_count to filter by age range
...
-- PART 3: Update get_available_users_at_place to filter by age range
...
BEGIN
  -- Get viewer's age preferences
  SELECT age_range_min, age_range_max
  INTO viewer_age_min, viewer_age_max
  FROM profiles
  WHERE id = viewer_id;
