-- Add get_active_users_with_avatars function and update RPCs with preview_avatars
--
-- This migration adds a unified function that returns both active_users count and preview_avatars
-- in a single query, replacing the need for separate calls.

-- Create composite type for returning count + avatars
...
-- Create unified function that returns both count and avatars
-- Uses profile_photos table (position = 0 for primary photo)
...
BEGIN
  ...
  -- Get avatar URLs for up to max_avatars eligible users
  -- Using profile_photos table with position = 0 for primary photo
  SELECT ARRAY(
    SELECT pp.url
    FROM user_presences up
  ...
  result.avatars := avatar_urls;
  
  RETURN result;
END;
$$;

-- Update search_places_nearby to return preview_avatars
...
BEGIN
  ...
  SELECT
    ...
    pv.total_checkins,
    pv.last_activity_at,
    (get_active_users_with_avatars(pv.id, requesting_user_id, 5)).count as active_users,
    (get_active_users_with_avatars(pv.id, requesting_user_id, 5)).avatars as preview_avatars,
    st_distance(
  ...
  ORDER BY
    ...
    CASE WHEN sort_by = 'relevance' THEN (get_active_users_with_avatars(pv.id, requesting_user_id, 5)).count END DESC,
...
-- Update get_trending_places to return preview_avatars
...
BEGIN
  ...
  WHERE
    ...
    AND (get_active_users_with_avatars(pv.id, requesting_user_id, 5)).count > 0
  ORDER BY active_users DESC, dist_meters ASC
  LIMIT max_results;
END;
$function$;

-- Update get_user_favorite_places to return preview_avatars
...
-- Update search_places_by_favorites to return preview_avatars
...
BEGIN
  ...
  SELECT
    ...
    p.total_score,
    (get_active_users_with_avatars(p.id, requesting_user_id, 5)).count as active_users,
    (get_active_users_with_avatars(p.id, requesting_user_id, 5)).avatars as preview_avatars,
    (
        SELECT count(*)
...
-- Update search_places_autocomplete to return preview_avatars
...
-- Update get_ranked_places to include preview_avatars
...
BEGIN
  ...
  WITH ranked AS (
    SELECT
      ...
      (get_active_users_with_avatars(p.id, requesting_user_id, 3)).avatars as preview_avatars
    FROM places_view p
    WHERE
      st_dwithin(
        st_setsrid(st_makepoint(p.lng, p.lat), 4326)::geography,
