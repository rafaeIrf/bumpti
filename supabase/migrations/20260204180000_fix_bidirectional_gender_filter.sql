-- =============================================================================
-- Migration: Fix bidirectional gender filter in active users functions
-- =============================================================================
-- This migration:
-- 1. Updates get_active_users_with_avatars to include bidirectional gender filter
-- 2. Updates get_eligible_active_users_count to include bidirectional gender filter
-- 3. Updates get_available_users_at_place to include bidirectional gender filter
-- 
-- Gender Filter Logic (Bidirectional):
-- RULE 1: Target user must want to connect with MY gender (already exists)
-- RULE 2 (NEW): I must want to connect with TARGET user's gender
-- =============================================================================
...
BEGIN
  ...
  WHERE up.place_id = target_place_id
    ...
    -- BIDIRECTIONAL gender preference RULE 1: Target user must want to connect with MY gender
    ...
    -- BIDIRECTIONAL gender preference RULE 2: I must want to connect with TARGET user's gender
    AND (requesting_user_id IS NULL OR EXISTS (
      SELECT 1 FROM profile_connect_with pcw
...
Filters: blocks, dislikes, likes, active matches, bidirectional gender preferences, bidirectional age filter, Trust Circle (bidirectional verification).
Gender: Respects connect_with preferences bidirectionally.';
...
Filters by: bidirectional gender preference compatibility, bidirectional age filter, Trust Circle (bidirectional verification).';
...
'Returns available users at a place for a viewer.
Excludes: self, blocked, disliked, liked, active matched users.
Filters by: bidirectional gender preference, bidirectional age filter.';
