-- RPC Function para retornar chats denormalizados para sync
-- Usado pela edge function sync-chat-data
  ...
  SELECT 
    ...
    CASE 
      WHEN um.user_a = p_user_id THEN (
        SELECT photo_url FROM user_photos WHERE user_id = um.user_b ORDER BY position LIMIT 1
      )
      ELSE (
        SELECT photo_url FROM user_photos WHERE user_id = um.user_a ORDER BY position LIMIT 1
      )
