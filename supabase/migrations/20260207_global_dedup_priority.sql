-- Global Deduplication by Priority
-- Problem: A user appearing in multiple encounter types (vibe_match + path_match)
-- would show up in multiple carousels on the Discover tab.
-- Solution: Two-pass CTE strategy:
--   1) per_type:   DISTINCT ON (other_user_id, encounter_type) — best encounter per type
--   2) prioritized: DISTINCT ON (other_user_id)                — ONE row per user, 
--                   using hierarchy: direct_overlap > vibe_match > routine/path
--
-- Metadata consolidation: the winning row carries an `additional_encounters` JSONB array
-- with the other encounter types + their place names, so the UI can show secondary context
-- (e.g. "Also goes to Starbucks Batel" on a vibe_match card).
