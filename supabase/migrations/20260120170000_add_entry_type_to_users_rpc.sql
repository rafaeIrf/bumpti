-- =============================================================================
-- Migration: Add entry_type to get_available_users_at_place return
-- =============================================================================
-- This allows clients to show different badges for physical vs checkin_plus users
...
RETURNS TABLE (
  ...
  languages text[],
  entry_type text  -- NEW: 'physical' or 'checkin_plus'
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  WITH active_presences AS (
    ...
    SELECT 
      ...
      up.entry_type AS u_entry_type  -- NEW
  ...
  SELECT
    ...
    COALESCE(eu.u_entry_type, 'physical')::text AS entry_type  -- NEW
