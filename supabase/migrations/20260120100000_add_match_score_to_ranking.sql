-- =============================================================================
-- Migration: Add composite ranking score (60% matches + 40% check-ins)
-- =============================================================================

-- Step 1: Add total_matches column to places table for caching match counts
ALTER TABLE places ADD COLUMN IF NOT EXISTS total_matches int DEFAULT 0;

-- Step 2: Create function to update match counts (for future use with triggers)
CREATE OR REPLACE FUNCTION update_place_match_count()
RETURNS TRIGGER AS $$
BEGIN
  -- Update match count for the place
  IF TG_OP = 'INSERT' AND NEW.place_id IS NOT NULL THEN
    UPDATE places SET total_matches = total_matches + 1 WHERE id = NEW.place_id;
  ELSIF TG_OP = 'DELETE' AND OLD.place_id IS NOT NULL THEN
    UPDATE places SET total_matches = GREATEST(0, total_matches - 1) WHERE id = OLD.place_id;
  END IF;
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;

-- Step 3: Create trigger to maintain match counts
DROP TRIGGER IF EXISTS update_place_match_count_trigger ON user_matches;
CREATE TRIGGER update_place_match_count_trigger
AFTER INSERT OR DELETE ON user_matches
FOR EACH ROW
EXECUTE FUNCTION update_place_match_count();

-- Step 4: Backfill skipped - only counting new matches going forward

-- Step 5: Update places_view to include total_matches
...
-- Step 6: Update get_ranked_places RPC with composite score
-- Formula: normalized_score = 0.6 * (matches / max_matches) + 0.4 * (checkins / max_checkins)
...
CREATE OR REPLACE FUNCTION get_ranked_places(
  ...
  rank_by text default 'composite', -- 'composite', 'matches', or 'checkins'
...
BEGIN
  ...
  WITH ranked AS (
    SELECT
      ...
      CASE 
        WHEN rank_by = 'matches' THEN 
          p.total_matches::float
        WHEN rank_by = 'checkins' THEN 
          p.total_checkins::float
        ELSE 
    ...
    WHERE
      ...
      -- Include places with either matches or checkins
      AND (p.total_matches > 0 OR p.total_checkins > 0)
  )
  SELECT
    r.id,
