-- =============================================================================
-- Migration: Prioritize nearby places in autocomplete + add neighborhood
-- =============================================================================
-- Changes:
--   1. Add dist_meters to ORDER BY so closer places appear first when
--      relevance scores are similar
--   2. Add neighborhood to returned columns for better UI display
-- =============================================================================
...
BEGIN
  ...
  WITH matched_places AS (
    SELECT
      ...
      get_regulars_count_at_place(p.id, requesting_user_id)::integer     as regulars_count,
...
COMMENT ON FUNCTION search_places_autocomplete IS 'Full text search for places with fuzzy matching, distance-prioritized ordering, active users count, neighborhood, and optional category filter';
