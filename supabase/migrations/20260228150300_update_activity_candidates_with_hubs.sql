-- =============================================================================
-- Migration: Add social hub notification types to get_place_activity_candidates
-- =============================================================================
-- Duplicates favorite_activity_started/heating for hubs (hub_activity_started,
-- hub_activity_heating) and adds profile_social_hubs exclusion to nearby blocks.
-- =============================================================================
...
BEGIN
  ...
  -- 3. HUB ACTIVITY STARTED (Count 1-2) — NEW: same logic for social hubs
  SELECT 
    hub.user_id AS target_user_id,
    'hub_activity_started'::text AS notification_type,
    ac.place_id AS target_place_id,
    ac.place_name AS target_place_name,
    ac.lat AS target_place_lat,
    ac.lng AS target_place_lng,
    ...
    ac.has_planning
  FROM active_counts ac
  JOIN profile_social_hubs hub ON hub.place_id = ac.place_id
  WHERE ac.count = 1
  AND NOT EXISTS (
    SELECT 1 FROM user_presences up_check 
    WHERE up_check.user_id = hub.user_id 
    AND up_check.place_id = ac.place_id 
    AND up_check.active = true
  )
  -- Exclude if also a favorite (avoid double notification)
  AND NOT EXISTS (
    SELECT 1 FROM profile_favorite_places fav_check
    WHERE fav_check.user_id = hub.user_id
  ...
  AND (
    ...
    OR EXISTS (
      ...
      FROM active_presences ap
      JOIN profile_connect_with pcw ON pcw.user_id = ap.user_id
      JOIN profiles target_prof ON target_prof.id = hub.user_id
      WHERE ap.place_id = ac.place_id
        AND pcw.gender_id = target_prof.gender_id
    )
  )
  AND (
    NOT EXISTS (SELECT 1 FROM profile_connect_with WHERE user_id = hub.user_id)
    OR EXISTS (
      SELECT 1 
      FROM active_presences ap
      JOIN profile_connect_with pcw ON pcw.user_id = hub.user_id
      WHERE ap.place_id = ac.place_id
        AND pcw.gender_id = ap.gender_id
    )
  )
  AND NOT EXISTS (
    SELECT 1 FROM notification_events ne
    WHERE ne.user_id = hub.user_id
    AND ne.place_id = ac.place_id
    AND ne.type = 'hub_activity_started'
    AND ne.created_at > NOW() - (ttl_started || ' hours')::interval
  )

  UNION ALL

  -- 4. HUB ACTIVITY HEATING (Count >= 2) — NEW: same logic for social hubs
  SELECT 
    hub.user_id AS target_user_id,
    'hub_activity_heating'::text AS notification_type,
    ac.place_id AS target_place_id,
    ac.place_name AS target_place_name,
    ac.lat AS target_place_lat,
    ac.lng AS target_place_lng,
  ...
  AND NOT EXISTS (
    ...
    WHERE ne.user_id = hub.user_id
    AND ne.place_id = ac.place_id
    AND ne.type = 'hub_activity_heating'
    AND ne.created_at > NOW() - (ttl_heating || ' hours')::interval
  )

  UNION ALL

  -- 5. NEARBY STARTED (Max 1 per user, Count 1-2, Within 50km radius)
  -- Now excludes BOTH favorites AND social hubs
  SELECT
    sub.target_user_id,
  ...
  FROM (
    ...
    -- Exclude favorites (handled by favorite rules)
    AND NOT EXISTS (
      SELECT 1 FROM profile_favorite_places fav_check
      WHERE fav_check.user_id = prof.id
      AND fav_check.place_id = ac.place_id
    )
    -- Exclude social hubs (handled by hub rules) — NEW
    AND NOT EXISTS (
      SELECT 1 FROM profile_social_hubs hub_check
      WHERE hub_check.user_id = prof.id
      AND hub_check.place_id = ac.place_id
    )
    AND (
      EXISTS (
        SELECT 1 FROM active_presences ap
  ...
  -- 6. NEARBY HEATING (Max 1 per user, Count >= 2, Within 50km radius)
  -- Now excludes BOTH favorites AND social hubs
  SELECT
    sub.target_user_id,
