-- Expand get_shared_favorite_users to include shared interests (vibes) matching
-- alongside existing shared favorite places matching.
-- Accepts optional lat/lng from client; falls back to profile's stored location.
-- Uses st_dwithin (50km radius) for interest-based matching when viewer has location.
...
BEGIN
  ...
  -- If viewer has coords: candidate must have coords AND be within 50km
  -- If viewer has NO coords: skip filter (can't calculate distance)
  ---------------------------------------------------------------------------
  int_matches AS (
    SELECT
