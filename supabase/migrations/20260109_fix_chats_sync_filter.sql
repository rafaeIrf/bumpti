...
BEGIN
  ...
  WITH chat_data AS (
    ...
    WHERE 
      ...
      AND c.first_message_at IS NOT NULL -- ← FILTRO CRÍTICO: Apenas chats COM mensagens
  )
  SELECT 
    cd.c_id,
    cd.c_match_id,
...
'Returns only chats WITH messages (first_message_at IS NOT NULL). 
Matches without messages are handled by fetchMatchesChanges.';
