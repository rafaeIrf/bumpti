-- =============================================================================
-- Migration: Add social hubs support to save_onboarding_txn
-- =============================================================================
-- Adds p_social_hub_ids uuid[] parameter to persist social hubs during
-- onboarding alongside favorite places.
-- =============================================================================
...
-- Drop the current version with photo_hashes
DROP FUNCTION IF EXISTS save_onboarding_txn(uuid, text, date, int, int[], int[], text[], int[], text[], uuid[], text, uuid, text, int, boolean, uuid[]);
...
CREATE OR REPLACE FUNCTION save_onboarding_txn(
  ...
  p_social_hub_ids uuid[] DEFAULT array[]::uuid[]  -- NEW: social hubs
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
begin
  ...
  -- Replace social hubs when provided
  delete from public.profile_social_hubs where user_id = p_user_id;
  if array_length(p_social_hub_ids, 1) is not null then
    insert into public.profile_social_hubs (user_id, place_id)
    select p_user_id, unnest(p_social_hub_ids);
  end if;
end;
$$;
