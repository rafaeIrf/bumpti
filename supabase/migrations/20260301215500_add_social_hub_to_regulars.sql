-- =============================================================================
-- Migration: Add social_hub as 4th source for regulars
-- =============================================================================
-- When a user sets a place as their social hub, they automatically become
-- a "regular" at that place. This has NO expiration — the user remains a
-- regular as long as the social hub entry exists in profile_social_hubs.
--
...
--   4. social_hub        — has this place as a social hub (never expires)
--
-- Base: 20260218180500_add_university_member_to_regulars.sql
-- =============================================================================
  ...
  RETURN QUERY
    WITH eligible_regulars AS (
      ...
      FROM (
        ...
        -- Source 4: Users who have this place as a social hub (NEVER expires)
        -- User remains a regular as long as the entry exists in profile_social_hubs.
        SELECT
          psh.user_id AS uid,
          'social_hub'::text AS etype,
          psh.created_at AS eat,
          4 AS priority  -- lowest priority
...
Sources: past check-ins (60 days) + favorites + university members + social hubs (never expires).
Active users excluded. Priority: past_visitor > favorite > university_member > social_hub.
