...
-- Updated to:
-- 1. Use profile_photos table (fixed in previous step)
-- 2. Remove updated_at dependency (chats table doesn't have it)
-- 3. Filter by last_message_at OR created_at for sync
...
BEGIN
  ...
  WHERE 
    p_since IS NULL 
    OR (
      -- If there is a last message, check its timestamp
      (cd.last_msg_at IS NOT NULL AND cd.last_msg_at >= p_since)
      -- OR check chat creation time (if no messages or new chat)
      OR (cd.c_created_at >= p_since)
    )
  ORDER BY 
