...
async function batchSignPhotos(
  ...
  const allSigned =
    allPaths.length > 0 ? await signPhotoUrls(supabase, allPaths) : [];
...
function buildUserResponse(
  ...
  return {
    ...
    entry_type: row.entry_type || "planning",
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const plannedFor =
      url.searchParams.get("planned_for") || bodyParams?.planned_for;
    const plannedPeriod =
      url.searchParams.get("planned_period") || bodyParams?.planned_period || null;

    if (!placeId || typeof placeId !== "string") {
      return jsonError("invalid_place_id", "place_id is required", 400);
    }
    if (!plannedFor || typeof plannedFor !== "string") {
      return jsonError("invalid_planned_for", "planned_for is required (YYYY-MM-DD)", 400);
    }
    // Validate ISO date format
    if (!/^\d{4}-\d{2}-\d{2}$/.test(plannedFor)) {
      return jsonError(
        "invalid_planned_for_format",
        "planned_for must be in YYYY-MM-DD format",
        400,
      );
    }
    ...
    // ── Parallel: planning users + likers ────────────────────────────────────
    const [rpcResult, likersResult] = await Promise.all([
      client.rpc("get_planning_users_at_place", {
        p_place_id: placeId,
        p_planned_for: plannedFor,
        p_planned_period: plannedPeriod,
        viewer_id: user.id,
      }),
      client
    ...
    const { data: plannerRows, error: rpcError } = rpcResult;
    if (rpcError) {
      console.error(
        "[get-planning-users] RPC get_planning_users_at_place failed:",
    ...
    console.log(
      `[get-planning-users] Planners: ${plannerRows?.length ?? 0} for place ${placeId} on ${plannedFor} ${plannedPeriod ?? "(any period)"}`,
    );
    ...
    if (likerIds.length > 0) {
      ...
      (matchRowsA ?? []).forEach(
        (r: any) => r.user_b && matchedIds.add(r.user_b),
      );
      (matchRowsB ?? []).forEach(
        (r: any) => r.user_a && matchedIds.add(r.user_a),
      );
      filteredLikerIds = likerIds.filter((id) => !matchedIds.has(id));
    }

    // ── Batch: sign photos + resolve favorite places ───────────────────────────
    const rows = (plannerRows || []) as PlannerRow[];

    const [signedUrlsMap, placeMap] = await Promise.all([
      batchSignPhotos(client, rows),
      resolveFavoritePlaces(client, rows),
    ]);
